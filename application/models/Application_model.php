...
class Application_model extends CI_Model
{
    ...
    public function onlineAdmissionProfilePic()
    {
        if (isset($_FILES["profile_pic"]) && !empty($_FILES['profile_pic']['name'])) {
            ...
            $allowedExts = array('jpg', 'jpeg', 'png');
            $extension = pathinfo($file_name, PATHINFO_EXTENSION);
            if ($files = filesize($_FILES['profile_pic']['tmp_name'])) {
                if (!in_array(strtolower($extension), $allowedExts)) {
                    $this->form_validation->set_message('profile_pic', translate('this_file_type_is_not_allowed'));
                    return false;
                }
                if ($file_size < 10000) {
                    $this->form_validation->set_message('profile_pic', translate('file_size_shoud_be_greater_than') . " 10KB.");
                    return false;
                }
                if ($file_size > 100000) {
                    $this->form_validation->set_message('profile_pic', translate('file_size_shoud_be_less_than') . " 100KB.");
                    return false;
                }
            } else {
                $this->form_validation->set_message('profile_pic', translate('error_reading_the_file'));
                return false;
            }
            return true;
        }
    ...
    public function onlineAdmissionProfileSign()
    {
        if (isset($_FILES["profile_sign"]) && !empty($_FILES['profile_sign']['name'])) {
            $file_size = $_FILES["profile_sign"]["size"];
            $file_name = $_FILES["profile_sign"]["name"];
            $allowedExts = array('jpg', 'jpeg', 'png');
            $extension = pathinfo($file_name, PATHINFO_EXTENSION);
            if ($files = filesize($_FILES['profile_sign']['tmp_name'])) {
                if (!in_array(strtolower($extension), $allowedExts)) {
                    $this->form_validation->set_message('profile_sign', translate('this_file_type_is_not_allowed'));
                    return false;
                }
                if ($file_size < 10000) {
                    $this->form_validation->set_message('profile_sign', translate('file_size_shoud_be_greater_than') . " 10KB.");
                    return false;
                }
                if ($file_size > 100000) {
                    $this->form_validation->set_message('profile_sign', translate('file_size_shoud_be_less_than') . " 100KB.");
                    return false;
                }
            } else {
                $this->form_validation->set_message('profile_sign', translate('error_reading_the_file'));
                return false;
            }
            return true;
        }
    ...
    {
        $sql = "SELECT e.*, s.photo, CONCAT(s.first_name, ' ', s.last_name) as fullname, s.register_no, s.parent_id, s.email, s.mobileno, s.blood_group, s.birthday, s.admission_date, l.active, l.username, c.name as class_name, se.name as section_name, sc.name as category FROM enroll as e INNER JOIN student as s ON e.student_id = s.id INNER JOIN login_credential as l ON l.user_id = s.id and l.role = 7 LEFT JOIN class as c ON e.class_id = c.id LEFT JOIN section as se ON e.section_id=se.id LEFT JOIN student_category as sc ON sc.id=s.category_id WHERE e.class_id = " . $this->db->escape($classID) . " AND e.branch_id = " . $this->db->escape($branchID) . " AND e.session_id = " . $this->db->escape(get_session_id());
        if ($sectionID != 'all') {
